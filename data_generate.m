...
H= zeros(nbrOfRealizations,M,K);
Hhat = zeros(nbrOfRealizations,M,K);
Herr= zeros(nbrOfRealizations,M,K);

%Generate uncorrelated Rayleigh fading channel realizations
h = (randn(nbrOfRealizations,M,K)+1i*randn(nbrOfRealizations,M,K));
%Go through all APs
for l = 1:M
    %Go through all pilots
    for t = 1:K
        H(:,l,t)=sqrt(BETAA(l,t))*h(:,l,t);
        Hhat(:,l,t)=sqrt(Gammaa(l,t))*h(:,l,t);
        Herr(:,l,t)=sqrt(BETAA(l,t)-Gammaa(l,t))*h(:,l,t);
    end
end
% gamaZF=zeros(M,K);
% %.'是普通转置，直接‘是H
...
%         gamaZF(:,k)=gamaZF(:,k)+conj(Herr(:,n,k))*Herr(:,n,k).'/nbrOfRealizations;
%     end
% end
...
save(['D:\desktop\work\code\Power_Allocation_DDPG-main\DDPG\datasetM100K10',num2str(n),'.mat']);
